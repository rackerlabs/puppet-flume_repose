...
# * Installation:
#
# class { 'flume-repose::valve': }
#
# * Removal/decommissioning:
#
# class { 'flume-repose::valve': ensure => 'absent' }
#
#
# === Authors
#
...
class flume-repose::valve (
  ...
  $autoupgrade       = $flume_repose::params::autoupgrade,
  $run_port          = $flume_repose::params::run_port,
  $daemon_home       = $flume_repose::params::daemon_home,
  $log_path          = $flume_repose::params::logdir,
  $pid_file          = $flume_repose::params::pid_file,
  ...
  file { '/etc/sysconfig/flume-repose':
  ...
  if $saxon_home { $saxon_ensure = present } else { $saxon_ensure = absent }
  $defaults = {
    'target'  => '/etc/sysconfig/flume_repose',
  }

  $vars = {
    'RUN_PORT'           => { 'value' => $run_port },
    'DAEMON_HOME'        => { 'value' => $daemon_home },
    'LOG_PATH'           => { 'value' => $log_path },
    'PID_FILE'           => { 'value' => $pid_file },
    'USER'               => { 'value' => $user },
    'daemonize'          => { 'value' => $daemonize },
    'run_opts'           => { 'value' => $run_opts },
    'java_opts'          => { 'value' => $java_opts },
    'JAVA_OPTS'          => { 'value' => $JAVA_OPTS },
    'SAXON_HOME'         => { 'value' => $saxon_home, 'ensure' => $saxon_ensure }
    'SAXON_HOME/export'  => { 'value' => '',          'ensure' => $saxon_ensure }
  }
  # write the sysconfig file. 
  create_resources(shellvar, $vars, $defaults)
